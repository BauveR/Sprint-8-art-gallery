  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <form onSubmit={onCreate} className="grid grid-cols-2 gap-3 bg-white/80 p-4 rounded-xl shadow">
        ...
        <Input placeholder="URL" value={form.url_tienda ?? ""} onChange={e => setForm(f => ({ ...f, url_tienda: e.target.value }))} />
        ...
        <div className="col-span-2"><Button disabled={createTienda.isPending}>{createTienda.isPending ? "Creando..." : "Crear tienda"}</Button></div>
      </form>

      <div className="bg-white/80 rounded-xl shadow overflow-x-auto">
        <table className="min-w-full text-sm">
          <thead className="bg-gray-50">
            <tr><th className="text-left p-2">#</th><th className="text-left p-2">Nombre</th><th className="text-left p-2">URL</th><th className="text-left p-2">Lat/Lng</th></tr>
          </thead>
          <tbody>
            {list.map(t => (
              <tr key={t.id_tienda} className="border-t">
                ...
                <td className="p-2">{t.url_tienda ? <a className="text-blue-600 underline" href={t.url_tienda} target="_blank">link</a> : "—"}</td>
                <td className="p-2">{t.lat}, {t.lng}</td>
              </tr>
            ))}
            {!isLoading && list.length === 0 && (<tr><td className="p-4 text-gray-500" colSpan={4}>Sin tiendas</td></tr>)}
          </tbody>
        </table>
